...
Some institutions or departments prohibit AI use outright. This approach is increasingly unenforceable as AI tools become embedded in standard software: browsers, word processors, email clients, search engines. More fundamentally, banning AI in an academic environment is pedagogically incoherent. Institutions that prepare students for professional practice cannot ignore the tools that define contemporary professional practice. The ban does not prevent AI use; it drives it underground, where it occurs without method, oversight, or disclosure.

### The Permissive Default

Other institutions permit AI use with minimal guidance, leaving individual faculty to set parameters. This creates inconsistency. Students encounter different rules in different courses, sometimes within the same program. Faculty who use AI in their own work have no shared method for maintaining intellectual ownership. The institution has no way to assess whether AI-assisted work meets its quality standards because those standards were developed before AI-assisted work existed.
...
Institutions need a method, not just rules about AI use. They need a structured approach that:
...
The Epistemic Stewardship Framework (ESF) provides this method. It is tool-agnostic (not dependent on any specific AI product), discipline-agnostic (applicable across programs), and designed for phased adoption within existing institutional structures.
...
- *Self-selection bias.* Faculty who volunteer for an AI methods pilot are likely already reflective about AI use. Phase 2 must address faculty who are less engaged.
...
- *Student resistance.* Some students will perceive the Five Questions and Reflection templates as unnecessary overhead. Framing the method as professional skill development (not compliance) is essential.
...
- Institutional AI policy adopted using ESF as the structural backbone
...
- Institution-wide AI policy document referencing ESF methods
...
3. **Community feedback** (mid-cycle): Gather input from faculty, students, and department coordinators on framework usability and effectiveness.
...
ESF does not replace existing institutional policies. It provides the structured process that makes those policies effective.
...
**ESF integration:** ESF provides the method that distinguishes responsible from irresponsible AI use. An academic integrity policy can reference ESF to clarify:
- AI use is permitted when it follows the ESF workflow (or an equivalent structured method)
...
This is not about penalizing AI use. It is about providing evidence that AI use was structured and sound. Faculty who use AI well should be recognized for it, not despite it.
...
- **Measure practice, not compliance.** The goal is to assess whether ESF is developing capacity (faculty methods, student agency), not whether boxes are checked.
...
1. **Identify a champion.** Find a faculty member, department chair, or center for teaching and learning director who is already thinking about AI integration methods. This person becomes the pilot coordinator.
